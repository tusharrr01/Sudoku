'use client';

import { useState } from 'react';
import { useRouter } from 'next/navigation';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import Link from 'next/link';
import { Spinner } from '@/components/ui/spinner';

export default function JoinGamePage() {
  const router = useRouter();
  const [playerName, setPlayerName] = useState('');
  const [roomCode, setRoomCode] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const handleJoin = async () => {
    if (!playerName.trim()) {
      setError('Please enter your name');
      return;
    }

    if (!roomCode.trim()) {
      setError('Please enter the room code');
      return;
    }

    setLoading(true);
    setError('');

    try {
      const response = await fetch('/api/games/join', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          roomCode: roomCode.toUpperCase().trim(),
          playerName: playerName.trim(),
        }),
      });

      if (!response.ok) {
        const data = await response.json();
        throw new Error(data.error || 'Failed to join game');
      }

      const data = await response.json();
      localStorage.setItem('sudoku-player-name', playerName.trim());
      if (data.playerId) localStorage.setItem('sudoku-player-id', data.playerId);
      router.push(`/game/${data.gameId}`);
    } catch (err: any) {
      setError(err.message || 'Failed to join game. Please check the code and try again.');
      console.error(err);
    } finally {
      setLoading(false);
    }
  };

  const handleKeyPress = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter' && playerName.trim() && roomCode.trim()) {
      handleJoin();
    }
  };

  return (
    <main className="min-h-[100dvh] bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-blue-100 via-white to-purple-100 dark:from-zinc-900 dark:via-zinc-950 dark:to-zinc-900 flex items-center justify-center p-4 sm:p-12 relative overflow-hidden">
      {/* Decorative Gradients */}
      <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-primary/20 rounded-full blur-[100px] -z-10 animate-float hidden md:block" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-accent/20 rounded-full blur-[100px] -z-10 animate-float delay-300 hidden md:block" />

      <div className="w-full max-w-lg relative z-10 animate-appear">
        {/* Back Button */}
        <Link href="/" className="inline-flex items-center text-primary hover:text-primary/80 mb-4 md:mb-8 transition-all hover:-translate-x-1 font-bold text-sm md:text-base">
          <span className="mr-2">←</span>
          Back to Home
        </Link>

        {/* Card */}
        <div className="glass-card rounded-3xl md:rounded-[2.5rem] p-6 md:p-12 border-2 border-white/40 shadow-2xl">
          <h1 className="text-3xl md:text-5xl font-black mb-1 md:mb-2 gradient-text tracking-tight uppercase italic mt-1">
            Join Session
          </h1>
          <p className="text-muted-foreground mb-6 md:mb-10 font-medium text-xs md:text-base">
            Enter your credentials and the room code to begin.
          </p>

          {/* Player Name Input */}
          <div className="mb-4 md:mb-8">
            <label className="block text-xs md:text-sm font-bold text-foreground/80 uppercase tracking-widest mb-2 md:mb-4">
              Your Name
            </label>
            <Input
              placeholder="What's your handle?"
              value={playerName}
              onChange={(e) => setPlayerName(e.target.value)}
              onKeyDown={handleKeyPress}
              disabled={loading}
              className="h-12 md:h-14 text-base md:text-lg rounded-xl md:rounded-2xl bg-white/50 border-2 border-zinc-200 hover:border-primary/50 focus:border-primary dark:border-white/10 dark:hover:border-primary/40 focus:ring-primary/30 font-medium transition-colors"
            />
          </div>

          {/* Room Code Input */}
          <div className="mb-6 md:mb-10">
            <label className="block text-xs md:text-sm font-bold text-foreground/80 uppercase tracking-widest mb-2 md:mb-4">
              Room Code
            </label>
            <Input
              placeholder="e.g., SOLVE8"
              value={roomCode}
              onChange={(e) => setRoomCode(e.target.value.toUpperCase())}
              onKeyDown={handleKeyPress}
              disabled={loading}
              maxLength={6}
              className="h-12 md:h-16 text-lg md:text-2xl font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-center rounded-xl md:rounded-2xl bg-white/50 border-2 border-zinc-200 hover:border-secondary/50 focus:border-secondary dark:border-white/10 dark:hover:border-secondary/40 focus:ring-secondary/30 text-secondary transition-colors"
            />
            <p className="text-[10px] md:text-xs text-muted-foreground mt-2 md:mt-3 font-medium text-center">
              Room codes are unique 6-character identifiers.
            </p>
          </div>

          {/* Error Message */}
          {error && (
            <div className="bg-destructive/10 border border-destructive/30 text-destructive px-5 py-4 rounded-2xl mb-8 text-sm font-bold animate-shake">
              {error}
            </div>
          )}

          {/* Join Button */}
          <Button
            onClick={handleJoin}
            disabled={loading || !playerName.trim() || !roomCode.trim()}
            className="w-full h-12 md:h-16 text-lg md:text-xl font-black bg-secondary hover:bg-secondary/90 text-secondary-foreground rounded-xl md:rounded-2xl shadow-xl hover:scale-[1.01] active:scale-[0.99] transition-all ring-offset-background hover:ring-4 ring-secondary/20"
          >
            {loading ? (
              <div className="flex items-center gap-2 md:gap-3">
                <Spinner className="w-4 h-4 md:w-5 md:h-5" />
                Connecting...
              </div>
            ) : (
              'Join Session'
            )}
          </Button>
        </div>
      </div>
    </main>
  );
}
